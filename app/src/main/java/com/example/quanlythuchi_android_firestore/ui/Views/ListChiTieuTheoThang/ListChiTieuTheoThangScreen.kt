...
fun ListChiTieuTheoThangScreen(
    ...
    val chitieuListState by chiTieuViewModel.getByThangVaNamState.collectAsState()

    LaunchedEffect(Unit) {
        chiTieuViewModel.getChiTieuTheoThangVaNam(userId, currentMonth, currentYear)
    }

    val chitieuList = when(chitieuListState){
        is UiState.Success -> (chitieuListState as UiState.Success).data
        else -> emptyList()
    }
    ...
    Scaffold(
        ...
        Box(
            ...
            when(chitieuListState){
                ...
                is UiState.Error -> {
                    Text("lỗi: ${(chitieuListState as UiState.Error).message}")
                }
                else -> {
                    DotLoading()
                }
